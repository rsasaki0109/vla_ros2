...
| `openvla` | VLA foundation model | available | verified: Built-in adapter metadata declares inputs, action shape, runtime mode, and caveats. | blocked: Local CUDA prompt probe found weights/deps but did not complete due to free VRAM. | planned: Health-first remote probe (vla-zoo remote-probe) is reproducible and verified against the dummy server; a recorded OpenVLA /v1/predict run on a GPU box is still needed. | planned: ROS2 remote command is generated; checked-in OpenVLA ROS2 action logs are still needed. | planned: PyBullet runner can call OpenVLA locally or remotely; heavy local run is skipped by default. | not_verified: No task-success or robot-skill claim is made for OpenVLA in this repository. |
| `pi0` | pi-family VLA | experimental | verified: Built-in adapter metadata declares inputs, action shape, runtime mode, and caveats. | planned: Needs a dedicated openpi or LeRobot serving environment and a recorded action probe. | planned: Remote-first deployment path is the intended initial route for pi0. | planned: ROS2 remote launch can target a pi0 server; checked-in action logs are still needed. | planned: Runner has the observation plumbing; real pi0 remote task traces are not checked in. | not_verified: No task-success or robot-skill claim is made for pi0/openpi in this repository. |
| `smolvla` | LeRobot policy | missing optional deps: pip install "vla_zoo[smolvla]" | verified: Built-in adapter metadata declares inputs, action shape, runtime mode, and caveats. | verified: CUDA inference-path probe completed with lerobot/smolvla_base. | planned: Isolated-env server plan is reproducible (vla-zoo smolvla-remote-plan); a recorded SmolVLA /v1/predict response is still needed. | planned: ROS2 remote launch can target a SmolVLA server; checked-in action logs are still needed. | partial: Rendered multi-camera plus state observation path was probed; not a task-success benchmark. | not_verified: SmolVLA base still needs robot/task-specific fine-tuning and calibration. |
| `groot` | humanoid/generalist foundation model | experimental | partial: Experimental adapter target is declared, but real GR00T inference is not implemented. | planned: Requires a dedicated NVIDIA GR00T stack and a recorded action probe. | planned: Expected to run through a remote serving environment once adapter support lands. | planned: ROS2 remote launch can target a GR00T server after real serving support exists. | not_verified: No real GR00T action traces are checked in. | not_verified: No task-success or robot-skill claim is made for GR00T in this repository. |
...
- Upstream: OpenVLA
- Next step: Run OpenVLA behind a GPU server with enough free VRAM and check in remote action logs.
...
| Local runtime | partial | Lazy Hugging Face adapter and prompt path are implemented without test-time downloads.<br>[adapter card](../adapters/openvla.md) |
| GPU inference | blocked | Local CUDA prompt probe found weights/deps but did not complete due to free VRAM.<br>[OpenVLA prompt probe](sample_task_verification/openvla_prompt_probe.md) |
| Remote server | planned | Health-first remote probe (vla-zoo remote-probe) is reproducible and verified against the dummy server; a recorded OpenVLA /v1/predict run on a GPU box is still needed.<br>[OpenVLA remote path](../openvla_remote.md), [remote probe tool sample](sample_task_verification/remote_probe_dummy.md) |
| ROS2 remote | planned | ROS2 remote command is generated; checked-in OpenVLA ROS2 action logs are still needed.<br>[ROS2 remote plan](ros2_remote_smoke_plan.md) |
| PyBullet tasks | planned | PyBullet runner can call OpenVLA locally or remotely; heavy local run is skipped by default.<br>[external adapter status](sample_task_verification/external_adapter_status.md) |
| Policy quality | not_verified | No task-success or robot-skill claim is made for OpenVLA in this repository.<br>- |
...
| Local runtime | blocked | Local inference is opt-in because checkpoint/config compatibility varies by LeRobot version.<br>[pi0 compatibility note](sample_task_verification/pi0_compatibility_probe.md) |
...
| Remote server | planned | Remote-first deployment path is the intended initial route for pi0.<br>[GPU server plan](sample_compare_suite/gpu_server_plan.md) |
...
- Next step: Record SmolVLA remote-server and ROS2 remote traces, then broaden task probes.
...
| Local runtime | verified | LeRobot SmolVLA adapter returned a typed action through load_model('smolvla').<br>[SmolVLA GPU probe](sample_task_verification/smolvla_gpu_probe.md) |
| GPU inference | verified | CUDA inference-path probe completed with lerobot/smolvla_base.<br>[SmolVLA GPU probe](sample_task_verification/smolvla_gpu_probe.md) |
| Remote server | planned | Isolated-env server plan is reproducible (vla-zoo smolvla-remote-plan); a recorded SmolVLA /v1/predict response is still needed.<br>[SmolVLA remote plan](smolvla_remote_smoke_plan.md), [env isolation](../smolvla_remote.md) |
| ROS2 remote | planned | ROS2 remote launch can target a SmolVLA server; checked-in action logs are still needed.<br>[ROS2 remote plan](ros2_remote_smoke_plan.md) |
| PyBullet tasks | partial | Rendered multi-camera plus state observation path was probed; not a task-success benchmark.<br>[SmolVLA PyBullet report](sample_task_verification/smolvla_pybullet_report.html) |
| Policy quality | not_verified | SmolVLA base still needs robot/task-specific fine-tuning and calibration.<br>- |

### groot

- Upstream: Isaac GR00T
- Next step: Replace the placeholder with a real GR00T serving adapter before claiming inference.
- Caveat: Experimental placeholder only; real GR00T inference is not verified in this repository yet.

| Cell | Status | Evidence |
|---|---|---|
| Contract | partial | Experimental adapter target is declared, but real GR00T inference is not implemented.<br>[adapter card](../adapters/groot.md) |
| Local runtime | blocked | Isaac GR00T dependencies are not installed or verified in this repository.<br>[external adapter status](sample_task_verification/external_adapter_status.md) |
| GPU inference | planned | Requires a dedicated NVIDIA GR00T stack and a recorded action probe.<br>- |
| Remote server | planned | Expected to run through a remote serving environment once adapter support lands.<br>[GPU server plan](sample_compare_suite/gpu_server_plan.md) |
