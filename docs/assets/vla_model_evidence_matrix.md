...
| `openvla` | VLA foundation model | available | verified: Built-in adapter metadata declares inputs, action shape, runtime mode, and caveats. | verified: 4-bit (nf4) loading fits a 16 GB consumer GPU: ~4.6 GB peak VRAM, ~1.1-2.7 s per inference. Measured via scripts/measure_openvla_runtime.py. | verified: A real `vla-zoo serve --model openvla --load-in-4bit` server passed a health-first probe and returned a typed 7-DoF action over HTTP /v1/predict (recorded end-to-end on a 16 GB GPU). | planned: ROS2 remote command is generated; checked-in OpenVLA ROS2 action logs are still needed. | planned: PyBullet runner can call OpenVLA locally or remotely; heavy local run is skipped by default. | not_verified: No task-success or robot-skill claim is made for OpenVLA in this repository. |
...
- Upstream: OpenVLA
- Next step: Local 4-bit GPU inference and a real remote /v1/predict are both verified; next, record a ROS2 remote action log, then add task-level probes.
