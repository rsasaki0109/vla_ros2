...
| Local runtime | blocked | Blocked until the NVIDIA Isaac GR00T stack is wired in; no GR00T inference ships and the adapter raises instead of fabricating actions. Reproducibly probed: load_model('groot').predict(...) raises MissingDependencyError, and no GR00T package is published to PyPI (gr00t/isaac-gr00t/nvidia-gr00t all 404) — the real runtime is the NVIDIA Isaac-GR00T GitHub stack, not a pip install.<br>[block probe](sample_task_verification/groot_block_probe.md), [blocked status](../groot_remote.md), [external adapter status](sample_task_verification/external_adapter_status.md) |
