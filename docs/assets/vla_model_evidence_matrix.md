...
| `openvla` | VLA foundation model | available | verified: Built-in adapter metadata declares inputs, action shape, runtime mode, and caveats. | blocked: Local CUDA prompt probe found weights/deps but did not complete due to free VRAM. | planned: GPU server command is generated; recorded OpenVLA /v1/predict run is still needed. | planned: ROS2 remote command is generated; checked-in OpenVLA ROS2 action logs are still needed. | planned: PyBullet runner can call OpenVLA locally or remotely; heavy local run is skipped by default. | not_verified: No task-success or robot-skill claim is made for OpenVLA in this repository. |
...
| Remote server | planned | GPU server command is generated; recorded OpenVLA /v1/predict run is still needed.<br>[GPU server plan](sample_compare_suite/gpu_server_plan.md) |
