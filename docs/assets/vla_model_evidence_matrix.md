...
- Next step: Local GPU inference is verified (~0.97 GB, ~60-130 ms); next, record remote-server and ROS2 remote traces, then broaden task probes.
...
| Local runtime | verified | lerobot/smolvla_base loaded and predicted a 6-DoF action through load_model('smolvla') on a local GPU, with measured load/VRAM/latency.<br>[local runtime evidence](../smolvla_local_runtime.md) |
