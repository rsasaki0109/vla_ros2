...
| `smolvla` | LeRobot policy | missing optional deps: pip install "vla_zoo[smolvla]" | verified: Built-in adapter metadata declares inputs, action shape, runtime mode, and caveats. | verified: CUDA inference measured at ~0.97 GB peak VRAM and ~60-133 ms steady latency (real-time capable). Captured via scripts/measure_lerobot_runtime.py. | verified: A real `vla-zoo serve --model smolvla` server passed a health-first probe and returned a typed 6-DoF action over HTTP /v1/predict (recorded end-to-end). | verified: The real VLARuntimeNode ran in remote mode against a live SmolVLA server: recorded 14 RemoteVLAClient actions + 106 status/diagnostics with 0 inference errors (vla-zoo ros remote-smoke-check passed). | partial: Rendered multi-camera plus state observation path was probed; not a task-success benchmark. | not_verified: SmolVLA base still needs robot/task-specific fine-tuning and calibration. |
...
- Next step: Local GPU, remote /v1/predict, and a ROS2 remote trace are all verified; next, broaden to task-level probes on real scene frames.
