...
from vla_ros2.core.errors import ConfigurationError, MissingDependencyError, UnknownModelError
from vla_ros2.core.model import BaseVLA

ENTRY_POINT_GROUP = "vla_ros2.adapters"
...
_BUILTINS: dict[str, AdapterInfo] = {
    ...
    "random": AdapterInfo(
        name="random",
        target="vla_ros2.adapters.baselines:RandomAdapter",
    ...
    "scripted": AdapterInfo(
        name="scripted",
        target="vla_ros2.adapters.baselines:ScriptedAdapter",
    ...
    "openvla": AdapterInfo(
        name="openvla",
        target="vla_ros2.adapters.openvla:OpenVLAAdapter",
        description="OpenVLA Hugging Face adapter.",
        install_hint='pip install "vla_ros2[openvla]"',
        metadata={
    ...
    "groot": AdapterInfo(
        name="groot",
        target="vla_ros2.adapters.groot:GR00TAdapter",
...
def load_model(
    ...
    **kwargs: Any,
    ...
    if runtime != "local":
        msg = (
            f"Unsupported runtime {runtime!r}; vla_ros2 supports on-robot 'local' "
            "inference only"
        )
        raise ConfigurationError(msg)
