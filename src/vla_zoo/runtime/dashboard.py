...
def _metadata(payload: Mapping[str, object]) -> dict[str, object]:
    ...
    if isinstance(raw, dict):
        return dict(raw)
    return _json_mapping_from_text(payload.get("metadata_json"))


def _diagnostic_values(payload: Mapping[str, object]) -> dict[str, object]:
...
def _iter_diagnostic_statuses(payload: object) -> list[Mapping[str, object]]:
    if isinstance(payload, dict) and isinstance(payload.get("status"), list):
        return [item for item in payload["status"] if isinstance(item, dict)]
    if isinstance(payload, dict) and {"level", "message"}.issubset(payload):
        return [payload]
    return []


def _looks_like_status_event(payload: object) -> bool:
    return isinstance(payload, dict) and (
        "ready" in payload or "status_text" in payload or "last_latency_ms" in payload
    )


def _looks_like_diagnostic_event(payload: object) -> bool:
    return bool(_iter_diagnostic_statuses(payload))


def _read_json_or_jsonl(path: Path) -> object:
...
def _payload_is_runtime_log(payload: object) -> bool:
    items = _payload_items(payload)
    return any(
        _looks_like_status_event(item) or _looks_like_diagnostic_event(item) for item in items
    )


def _aggregate_runtime_sample(
    aggregates: dict[tuple[str, str, str | None], dict[str, object]],
    *,
    model_name: str,
    runtime: str,
    source: str,
    ...
    fallback_avg_latency_ms: float | None = None,
) -> None:
    key = (model_name, runtime, remote_url)
    ...
    aggregate["samples"] = _int(aggregate["samples"]) + 1
    aggregate["latest_ok"] = ok
    aggregate["latest_status_text"] = status_text
    if source and source not in _string(aggregate["source"]):
        aggregate["source"] = f"{aggregate['source']},{source}" if aggregate["source"] else source
    if not ok:
        aggregate["errors"] = _int(aggregate["errors"]) + 1
    latency = latency_ms if latency_ms is not None else fallback_avg_latency_ms
    if latency is not None:
        aggregate["queries"] = _int(aggregate["queries"]) + 1
        latencies = aggregate["latencies"]
        if isinstance(latencies, list):
            latencies.append(latency)


def _runtime_status_samples_from_payload(
    payload: object,
    *,
    source: str = "",
    ...
    for item in _payload_items(payload):
        if not isinstance(item, dict) or not _looks_like_status_event(item):
            continue
        metadata = _metadata(item)
        model_name = _string(item.get("model_name", metadata.get("model_name", "unknown")))
        runtime = _string(item.get("runtime", metadata.get("runtime", "ros2")), "ros2")
        status_text = _string(item.get("status_text", metadata.get("status_text", "")))
        samples.append(
            {
                "model_name": model_name,
                "runtime": runtime,
                "source": source,
                "remote_url": (
                    _string(metadata.get("remote_url"))
                    if metadata.get("remote_url") is not None
                    else None
                ),
                "ok": _bool(item.get("ready", metadata.get("ready", False))),
                "status_text": status_text,
                "latency_ms": _float(item.get("last_latency_ms")),
                "avg_latency_ms": _float(item.get("avg_latency_ms")),
            }
        )
    return samples


def _runtime_diagnostic_samples_from_payload(
    payload: object,
    *,
    source: str = "",
) -> list[dict[str, object]]:
    samples: list[dict[str, object]] = []
    for item in _payload_items(payload):
        for status in _iter_diagnostic_statuses(item):
            values = _diagnostic_values(status)
            level = _int(status.get("level"))
            model_name = _string(
                values.get("model_name", status.get("hardware_id", "diagnostics")),
                "diagnostics",
            )
            runtime = _string(values.get("runtime", "ros2"), "ros2")
            samples.append(
                {
                    "model_name": model_name,
                    "runtime": runtime,
                    "source": source,
                    "remote_url": (
                        _string(values.get("remote_url"))
                        if values.get("remote_url") is not None
                        else None
                    ),
                    "ok": level == 0,
                    "status_text": _string(status.get("message", "")),
                    "latency_ms": _float(values.get("last_latency_ms")),
                    "avg_latency_ms": _float(values.get("avg_latency_ms")),
                }
            )
    return samples


def dashboard_records_from_runtime_log_payload(
    payload: object,
    *,
    source: str = "",
) -> list[ComparisonDashboardRecord]:
    return _dashboard_records_from_runtime_samples(
        [
            *_runtime_status_samples_from_payload(payload, source=source),
            *_runtime_diagnostic_samples_from_payload(payload, source=source),
        ]
    )


def _dashboard_records_from_runtime_samples(
    samples: list[dict[str, object]],
) -> list[ComparisonDashboardRecord]:
    aggregates: dict[tuple[str, str, str | None], dict[str, object]] = {}
    for sample in samples:
        _aggregate_runtime_sample(
            aggregates,
            model_name=_string(sample.get("model_name", "unknown"), "unknown"),
            runtime=_string(sample.get("runtime", "ros2"), "ros2"),
            source=_string(sample.get("source")),
            remote_url=(
                _string(sample.get("remote_url")) if sample.get("remote_url") is not None else None
            ),
            ok=_bool(sample.get("ok")),
            status_text=_string(sample.get("status_text")),
            latency_ms=_float(sample.get("latency_ms")),
            fallback_avg_latency_ms=_float(sample.get("avg_latency_ms")),
        )
    ...
    for aggregate in aggregates.values():
        ...
        numeric_latencies = (
            [float(value) for value in latencies] if isinstance(latencies, list) else []
        )
        latest_ok = _bool(aggregate.get("latest_ok"))
        latest_status_text = _string(aggregate.get("latest_status_text"))
        records.append(
            ComparisonDashboardRecord(
                model_name=_string(aggregate.get("model_name"), "unknown"),
                runtime=_string(aggregate.get("runtime"), "ros2"),
                ok=latest_ok,
                source=_string(aggregate.get("source")),
                remote_url=(
                    _string(aggregate.get("remote_url"))
                    if aggregate.get("remote_url") is not None
                    else None
                ),
                frames=_int(aggregate.get("samples")),
                adapter_queries=_int(aggregate.get("queries")),
                adapter_errors=_int(aggregate.get("errors")),
                mean_latency_ms=(
                    sum(numeric_latencies) / len(numeric_latencies) if numeric_latencies else None
                ),
                max_latency_ms=max(numeric_latencies) if numeric_latencies else None,
                mean_abs_action=None,
                last_error=None if latest_ok else latest_status_text or "runtime unhealthy",
            )
        )
    return sorted(records, key=lambda record: (record.model_name, record.runtime))
...
def dashboard_records_from_payload(
    ...
    *,
    ...
    if _payload_is_runtime_log(payload):
        return dashboard_records_from_runtime_log_payload(payload, source=source)
...
def load_runtime_dashboard_records(paths: list[Path]) -> list[ComparisonDashboardRecord]:
    samples: list[dict[str, object]] = []
    for path in paths:
        payload = _read_json_or_jsonl(path)
        samples.extend(_runtime_status_samples_from_payload(payload, source=str(path)))
        samples.extend(_runtime_diagnostic_samples_from_payload(payload, source=str(path)))
    return _dashboard_records_from_runtime_samples(samples)
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .scope-card.warn { border-left-color: var(--warn); background: #fffbeb; }
    .scope-card.ok-scope { border-left-color: var(--ok); background: #f0fdf4; }
    .scope-card h2 {
      margin: 0 0 6px;
      font-size: 15px;
    }
...
<body>
  <main>
    ...
    <section class="scope">
      <div class="scope-card ok-scope">
        <h2>What This Shows</h2>
        <p>Runtime evidence from comparison JSON or ROS2 logs: readiness, queries,
          latency, adapter errors, task metrics, and action magnitude.</p>
      </div>
      <div class="scope-card warn">
        <h2>What This Does Not Claim</h2>
        <p>This dashboard is not a VLA model-quality leaderboard and does not prove
          real-robot task success without hardware-specific validation.</p>
      </div>
      <div class="scope-card">
        <h2>How To Read It</h2>
        <p>Use the matrix first, then triage degraded rows. Keep ROS2 dry-run,
          watchdogs, and action clipping in the deployment path.</p>
      </div>
    </section>
    ...
    <section class="grid">
      <div class="panel wide">
        <h2>Runtime Evidence Matrix</h2>
        <div class="panel-body" id="evidenceMatrix"></div>
      </div>
      <div class="panel">
        <h2>Fleet Health</h2>
  ...
  <script>
    ...
    function renderEvidenceMatrix(rows) {
      const sorted = [...rows].sort((a, b) => {
        const model = text(a.model_name).localeCompare(text(b.model_name));
        if (model !== 0) return model;
        return text(a.runtime).localeCompare(text(b.runtime));
      });
      document.getElementById("evidenceMatrix").innerHTML = sorted.length
        ? `<table class="matrix-table">
          <thead>
            <tr>
              <th>Adapter</th><th>Runtime</th><th>Status</th><th>Health</th>
              <th>Task</th><th>Latency</th><th>Errors</th><th>Action</th><th>Evidence</th>
            </tr>
          </thead>
          <tbody>
            ${sorted.map(record => `
              <tr>
                ...
                <td>
                  <div class="signal">
                    <strong>${healthScore(record)}%</strong>
                    <small>${esc(recordState(record))}</small>
                  </div>
                </td>
                <td>${esc(taskLabel(record))}</td>
                <td>${fmt(record.mean_latency_ms)} ms</td>
                <td>${record.adapter_errors}/${record.adapter_queries}</td>
                <td>${fmt(record.mean_abs_action, 3)}</td>
                <td class="note">
                  ${esc(record.source || record.last_error || "generated record")}
                </td>
              </tr>
            `).join("")}
          </tbody>
